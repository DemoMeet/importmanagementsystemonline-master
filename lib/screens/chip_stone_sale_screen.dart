...
import 'chip_stone_pur_entry_screen.dart';
import 'chip_stone_sale_entry_screen.dart';
import 'crusher_sale_entry_screen.dart';
import 'crusher_sale_update.dart';
import 'dashboard.dart';
import 'individual_lc_entry_screen.dart';

class ChipStoneSaleScreen extends StatefulWidget {
  const ChipStoneSaleScreen({Key? key}) : super(key: key);

  @override
  _ChipStonePurScreenState createState() => _ChipStonePurScreenState();
}

class _ChipStonePurScreenState extends State<ChipStoneSaleScreen> {
  ...
  void initState() {
    ...
    //             .toStringAsFixed(3));
    //         _oneToSix = double.parse((double.parse(_oneToSix.toString()) +
      ...
      for (var doc in querySnapshot.docs) {
          setState(() {
            _totalStock = double.parse((double.parse(_totalStock.toString()) +
                ...
                .toStringAsFixed(3));
            _totalSale = (double.parse(_totalSale.toString()) +
        ...
        SpeedDialChild(
            ...
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => ChipStoneSalentryScreen()));
            },
            label: 'Add Data',
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [nameSearchField, yearSearchField],
            ),
          ),
          Padding(
            padding: const EdgeInsets.all(8.0),
  ...
  final CollectionReference _collectionReference =
      FirebaseFirestore.instance.collection("cStoneSales");
  ...
  //     for (var doc in querySnapshot.docs) {
  //
  //         if (yearSearchController.text.isNotEmpty &&
  ...
  //             yearSearchController.text.isEmpty) {
  //           if (doc["buyerName"]
  //               .toString()
  //               .toLowerCase()
